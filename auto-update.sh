...
match_and_download() {
    ...
    # 包类型列表：类型|日志名称
    local types="arch|架构包 luci|Luci包 lang|语言包"
    
    for type_info in $types; do
        local pkg_type=$(echo "$type_info" | cut -d'|' -f1)
        local label=$(echo "$type_info" | cut -d'|' -f2)
            ...
            for arch in $ARCH_FALLBACK; do
                ...
                for filename in $all_files; do
                    ...
                    if echo "$filename_lower" | grep -q "$arch" && echo "$filename_lower" | grep -q "$app_name_lower"; then
                        log "  [$label] $filename (匹配: $arch)"
                        download_and_install_single "$filename" && success_count=$((success_count + 1)) && arch_found=1
                        break
                    fi
                done
                IFS="$old_IFS"
